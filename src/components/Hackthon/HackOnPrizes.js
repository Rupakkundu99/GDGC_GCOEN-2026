...
const PrizeCard = ({ trophy, bg, title, amount, height, trophyOffset, titleColor, amountColor }) => {
  return (
    <div className="relative flex flex-col items-center">
      ...
      {/* Card */}
      <div
        className="relative w-[220px] md:w-[260px] rounded-[28px] overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.35)]"
        style={{ height }}
      >
        ...
        <div className="absolute bottom-0 left-0 right-0 p-5 md:p-6 text-center">
          <p className={`text-xl md:text-2xl font-semibold ${titleColor}`}>
            {title}
          </p>
          <p className={`text-3xl md:text-4xl font-bold mt-2 ${amountColor}`}>
            {amount}
          </p>
        </div>
      </div>
...
const HackOnPrizes = () => {
  return (
    <section className="w-full relative py-10 md:py-16 bg-black">
      ...
      <div className="mx-auto flex items-end justify-center gap-6 md:gap-10">
  {/* 2nd Prize */}
  <PrizeCard
  ...
  />
      ...
      {/* Decorative pin like original */}
      <Image
        src="/Pin.svg"
        alt="pin"
        width={160}
        height={160}
        className="absolute left-0 -bottom-8 w-24 md:w-48"
      />
    </section>
  );
};
